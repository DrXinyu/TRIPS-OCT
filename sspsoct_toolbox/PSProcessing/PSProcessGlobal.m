function out = PSProcessGlobal(S1,S2,procStruct)
%out = PSProcessGlobal(S1,S2,procStruct) - computes local birefringence of 
%the Stokes vectors S1 and S2, corresponding to the Stokes vectors measured 
%for an input polarization state modulated between states orthogonal on the
%Poincaree sphere, using spectral binning. In contrast to PSPRocess, which
%estimates the alignment between the spectral bins for each A-line,
%PSProcessGlobal computes a constant alignment across the entire B-scan.
...
B = bsxfun(@times,PA(:,:,mid,:),mask);

for wind = [(1:mid-1),(mid+1:dim(3))]
    A = bsxfun(@times,PA(:,:,wind,:),mask);
