...
%             intInd(intInd<0.4) = 0;  
            ...
            for index = 1:n
                ...
                if W ~= length(surface)
                    surface =  round(interp1(1:length(surface),surface,linspace(1,length(surface),W)));
                end
        ...
        function image = write_intensity_image_with_sclera(self,int,int_r,layer_data)
        ...
        function image = write_intensity_image_with_RNFL(self,int,int_r,layer_data)
...
%             intInd(intInd<0.5) = 0;  
        ...
        function image = write_color_ret_image(self,int,ret,int_r,ret_r,eql)
            if nargin < 6
                eql = 1;
            end
            ...
            if eql
                ...
                %equlizedInd = intInd;
                equlizedInd = histeq(intInd);
                %equlizedInd(equlizedInd<0.5) = 0;
                equlizedInd(equlizedInd<0.6) = 0;
%             
            else
            ...
            dnRGB = ind2rgb(uint8(dnInd*255),flipud(aaa.weak_ametrine));
            %dnRGB = ind2rgb(uint8(dnInd*255),aaa.my_turbo);
