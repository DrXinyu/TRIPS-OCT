function [retout,dopout,varargout] = gpuPSProcess(S1,S2,fwx,dz,Nw,prepad,postpad,resz)
...
% of the input Stokes vectors is Nk,NAlines,Nw,[Q,U,V]
%
% Arguments:
...
% filtering with h
Norig = size(S1,1);
Nfft = 2^ceil(log2(Norig));
S1 = conv2(S1(:,:),h,'same');
S2 = conv2(S2(:,:),h,'same');
I1 = conv2(I1(:,:),h,'same');
I2 = conv2(I2(:,:),h,'same');
%S1 = bsxfun(@times,fft(S1,Nfft,1),fft(h,Nfft,1));
%S1 = ifft(S1,Nfft,1,'symmetric');
%S1 = S1(1:Norig,:,:,:);

%S2 = bsxfun(@times,fft(S2,Nfft,1),fft(h,Nfft,1));
%S2 = ifft(S2,Nfft,1,'symmetric');
%S2 = S2(1:Norig,:,:,:);

%I1 = bsxfun(@times,fft(I1,Nfft,1),fft(h,Nfft,1));
%I1 = ifft(I1,Nfft,1,'symmetric');
%I1 = I1(1:Norig,:,:,:);

%I2 = bsxfun(@times,fft(I2,Nfft,1),fft(h,Nfft,1));
%I2 = ifft(I2,Nfft,1,'symmetric');
%I2 = I2(1:Norig,:,:,:);
S1 = reshape(S1,dimIn);
S2 = reshape(S2,dimIn);
I1 = reshape(I1,dimIn(1:end-1));
I2 = reshape(I2,dimIn(1:end-1));
...
% final normalization
S1 = bsxfun(@rdivide,S1,sqrt(L1));
S2 = bsxfun(@rdivide,S2,sqrt(L2));
...
%    C = zeros([3,3,dim(1)],'single');
%    C(1,:,:) = conv2(squeeze(sum(bsxfun(@times,PA(:,:,wind,:),ref(:,:,1)),2)),h,'same');
%    C(2,:,:) = conv2(squeeze(sum(bsxfun(@times,PA(:,:,wind,:),ref(:,:,2)),2)),h,'same');
%    C(3,:,:) = conv2(squeeze(sum(bsxfun(@times,PA(:,:,wind,:),ref(:,:,3)),2)),h,'same');
