function out = exportOAPSTif(path,fout,fwx,dz,logLim,maxRet,N,opt,boolCatheter)
...
st = struct;
st.bgr = 'median';% Set bgr to use median signal to avoid findCatheter from failing
[m,d] = readConfig(path);
dzres = 4.33;
boolphi2 = false;
...
for ind = 1:numel(sliceInd)
    ...
    if oop==1
        ...
        if ~isempty(inds)
            [s1w,s2w] = recstrTom(path,[inds(1),inds(end)],st2);
    ...
    if boolCatheter
        out = OAProcessingCath(S1w,S2w,int,pstruct,1);
    else
        out = OAProcessingBT(S1w,S2w,int,pstruct,1);
    end
    
    byteString = writeToUint8(out);
%    oaStr = readFromUint8(byteString,9,512)
    
    intout = uint8(255*(10*log10(out.int)-logLim(1))/diff(logLim));
